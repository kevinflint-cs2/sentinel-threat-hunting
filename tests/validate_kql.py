...
class KQLValidator:
    ...
    def _validate_query(self, query: str, query_num: int):
        ...
        for limit in make_set_matches:
            if int(limit) > 100:
                self.warnings.append(
                    f"Query {query_num}: make_set limit of {limit} may be excessive"
                )
